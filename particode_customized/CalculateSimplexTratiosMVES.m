function minRandArchRatio=CalculateSimplexTratiosMVES(DataPoints,NumArch,maxRuns,numIter)
...
% numIter - number of iterations to run per dataset
...
%     end
   ...
   % [~ , VolConvRand(m)]=convhulln(SimplexRand1(:,1:Nmembers-1));
    ...
    for k=1:numIter
        % MVES algorithm
        [Arch3Rand,~, ~, ~] = MVES(SimplexRand1',Nmembers,0);
        % Calculating the volume of the randomized simplex
        if ~isnan(Arch3Rand)
            ArchRandRed=bsxfun(@minus,Arch3Rand,Arch3Rand(:,Nmembers));
            VolArchRand(k)=abs(det(ArchRandRed(1:end,1:end-1))/factorial(Nmembers-1));
