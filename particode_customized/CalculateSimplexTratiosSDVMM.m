function minRandArchRatio=CalculateSimplexTratiosSDVMM(DataPoints,NumArch,maxRuns,numIter)
...
% numIter - number of iterations per dataset run (not used)
...
%     end
   ...
   % [~ , VolConvRand(m)]=convhulln(SimplexRand1(:,1:Nmembers-1));
    ...
    for k=1:numIter
        % SDVMM algorithm
        [Arch3Rand,~] = SDVMM(SimplexRand1',Nmembers,0);
